...
fn benchmark_contextgraph() -> std::time::Duration {
    ...
    println!("  - Added {edge_count} edges in {:.3}s", edge_start.elapsed().as_secs_f64());

    // Simple query: find components with labels
    let query_start = Instant::now();
    let labeled_nodes = graph.query_nodes_with_component::<Label>();
    println!("  - Found {labeled_nodes.len(} nodes with labels in {:.6}s"), query_start.elapsed().as_secs_f64());
    ...
    let labeled_nodes = graph.query_nodes_with_component::<Label>();
    println!("  - Nodes with labels: {labeled_nodes.len(} (queried in {:.6}s)"),
